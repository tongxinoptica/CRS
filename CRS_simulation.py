...
scatter = scatter_amp * np.exp(1j*scatter_phase)
...
# 绘制相位结果
plt.figure(figsize=(8, 8))
plt.imshow(phase_avg, extent=(-physical_size / 2, physical_size / 2, -physical_size / 2, physical_size / 2), cmap='jet')
plt.colorbar()
...
# 绘制空间相干性
plt.figure(figsize=(8, 8))
plt.imshow(coherence,cmap='jet')
cbar = plt.colorbar()
cbar.set_ticks([np.min(coherence), np.max(coherence)])
# plt.title('Spatial Coherence after Rotating Scattering Plate and Pin-hole')
# plt.xlabel('x (m)')
# plt.ylabel('y (m)')
plt.show()
plt.savefig(f'R={pinhole_radius}.png', bbox_inches='tight', dpi=300)
